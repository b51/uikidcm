...
function Set_Brightness()
  ...
  local expo = Camera.get_param('exposure (absolute)');
  local gain = Camera.get_param('gain');
  ...
  Camera.set_param ('exposure, auto priority', 0);
--  Camera.set_param ('White Balance, Automatic', 0);
  Camera.set_param ('exposure (absolute)', expo);
  Camera.set_param ('gain', gain);
...
function Set_Exposure()
  print ('Current exposure is: '..Camera.get_param('exposure (absolute)')..' type in a integer between 0 and 2800');
  local param = Read_Num();
  ...
  --Camera.set_param ('White Balance, Automatic', 0);
  --Camera.set_param ('Auto Exposure', 1)
  Camera.set_param ('exposure (absolute)', param);
  --Camera.set_param ('Gain', backup);
  --Camera.set_param ('White Balance, Automatic', 0);
  --Camera.set_param ('Auto Exposure', 0);
  print('Exposure: ' , Camera.get_param('exposure (absolute)'))
end
...
function Print_All()
  ...
  for c=1,Config.camera.ncamera do
    ...
    print ('Auto Exposure: '..Camera.get_param('exposure, auto'))
    print ('White Balance, Automatic: '..Camera.get_param('white balance temperature'))
    print ('');
  end
