...
--          r_reject = 1.0;
  ...
  walk.set_velocity(vx,vy,va);
	...
	elseif ownGoal[1] < 0 then
		print("ball in global: "..ballGlobal[1]);
		if ballGlobal[1] > 0 then
			if walk.active then
				walk.stop();
				HeadFSM.sm:set_state('headTrack');
...
--	print(ballR,rClose);
  ...
  if math.abs(homeRelative[1])<thClose[1] and
    ...
    ballR<rClose and
    t-ball.t<tBall then
      print("homeRelative, ballR :",homeRelative[1], homeRelative[2], homeRelative[3]*180/math.pi, ballR);
