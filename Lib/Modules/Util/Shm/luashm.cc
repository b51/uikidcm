...
static int lua_shm_create(lua_State* L) {
  ...
  managed_shared_memory** ud = (managed_shared_memory**)lua_newuserdata(
      L, sizeof(managed_shared_memory*));
...
static int lua_shm_create_empty_variable(lua_State* L) {
  ...
  if (pr == NULL) {
    ...
    //    printf("WARNING: Input size %d != current block size %d. Resizing %s
    //    block.\n", nval, n, key);
    shm->destroy_ptr(pr);
    pr = shm->construct<value_t>(key)[nval]();
  }
...
static int lua_shm_tostring(lua_State* L) {
  managed_shared_memory* shm = lua_checkshm(L, 1);
  lua_pushfstring(L, "shm(%p): %d bytes, %d free, %d named, %d unique", shm,
                  shm->get_size(), shm->get_free_memory(),
                  shm->get_num_named_objects(), shm->get_num_unique_objects());
  return 1;
}

static const struct luaL_Reg shm_functions[] = {{"new", lua_shm_create},
                                                {"open", lua_shm_open},
...
int luaopen_shm(lua_State* L) {
  ...
  */
  ...
  luaL_setfuncs(L, shm_functions, 0);
