...
--打包各参数传递给DSP模块，这部分需要商议
--对table的用法，lua调用c语言，c语言调用lua仍然不太清楚，需要学习
[[
SpecialGaitCommand={id=0,times=0};--特殊步态
HeadMovingCommand={pitch=0,yaw=0};--头部实际转角
GaitEffect={xOffset=0,yOffset=0,thetaOffset=0};--
Sensors={};
--下发数据包
StateSwapInput={ctrReg1,ctrReg2,dirInst=GaitEffect,spcInst=SpecialGaitCommand,headInst=HeadMovingCommand};
--返回数据包
StateSwapOutput={stsReg1,stsReg2,dirSts=GaitEffect,spcSts=SpecialGaitCommand,headSts=HeadMovingCommand,
odometer=GaitEffect,sensors=Sensors};
--Instruction packet:Dynamixel ID,packet length,instruction code, parameters
Dsp_Inst_Packet={id,length,instruction,parameter={}};
--Status packet:Dynamixel ID,packet length,Error code,parameters
Dsp_Status_Packet={id,length,infomation,parameter={}};
]]
-- Setup shared memory
function shm_init()
