...
int v4l2_init(int w, int h) {
  ...
  // Query V4L2 controls:
  int addr_end = 22;
  v4l2_query_ctrl(V4L2_CID_BASE, V4L2_CID_LASTP1);
  v4l2_query_ctrl(V4L2_CID_PRIVATE_BASE, V4L2_CID_PRIVATE_BASE + 20);
  v4l2_query_ctrl(V4L2_CID_CAMERA_CLASS_BASE + 1,
                  V4L2_CID_CAMERA_CLASS_BASE + addr_end);

  // Logitech specific controls:
  v4l2_query_ctrl(V4L2_CID_FOCUS, V4L2_CID_FOCUS + 1);
  v4l2_query_ctrl(V4L2_CID_LED1_MODE, V4L2_CID_LED1_MODE + 1);
  v4l2_query_ctrl(V4L2_CID_LED1_FREQUENCY, V4L2_CID_LED1_FREQUENCY + 1);
  v4l2_query_ctrl(V4L2_CID_DISABLE_PROCESSING, V4L2_CID_DISABLE_PROCESSING + 1);
  v4l2_query_ctrl(V4L2_CID_RAW_BITS_PER_PIXEL, V4L2_CID_RAW_BITS_PER_PIXEL + 1);

  // hack
  v4l2_query_ctrl(V4L2_CID_BASE, V4L2_CID_BASE + 500);
  ...
  while (0 == ioctl(video_fd, VIDIOC_QUERYCTRL, &queryctrl)) {
    if (!(queryctrl.flags & V4L2_CTRL_FLAG_DISABLED)) {
      fprintf(stdout, "Control %s: ", queryctrl.name);

      if (queryctrl.type == V4L2_CTRL_TYPE_MENU)
        enumerate_menu(queryctrl.id);
      ioctl(video_fd, VIDIOC_QUERYCTRL, &queryctrl);
...
int v4l2_get_width() { return width; }

int v4l2_get_height() { return height; }
